...
const validatePlayerCreation = [
  ...
  body('baseValue')
    .isNumeric()
    .withMessage('Base value must be a number')
    .isInt({ min: 1 })
    .withMessage('Base value must be a positive integer'),
...
const validateAuctionCreation = [
  ...
  body('categories')
    ...
    .custom((categories) => {
      const validCategories = ['GK', 'DEF', 'MID', 'ATT'];
  ...
  body('categoryFlow')
    .isArray({ min: 1 })
    .withMessage('Category flow must be specified'),
  ...
  body('timerDuration')
    .optional()
    .isNumeric()
    .withMessage('Timer duration must be a number')
    .isInt({ min: 10, max: 300 })
    .withMessage('Timer duration must be between 10 and 300 seconds'),
...
const validateBroadcast = [
  ...
  body('type')
    .optional()
    .isIn(['general', 'announcement', 'auction', 'system'])
...
const validateSettings = [
  body('auctionTimer')
    .optional()
    .isNumeric()
    .withMessage('Auction timer must be a number')
    .isInt({ min: 10, max: 300 })
    .withMessage('Auction timer must be between 10 and 300 seconds'),

  body('breakTimer')
    .optional()
    .isNumeric()
    .withMessage('Break timer must be a number')
    .isInt({ min: 0, max: 120 })
    .withMessage('Break timer must be between 0 and 120 seconds'),

  body('baseBudget')
    .optional()
    .isNumeric()
    .withMessage('Base budget must be a number')
    .isInt({ min: 1000000 })
    .withMessage('Base budget must be at least 1,000,000'),

  body('ruleTillValue')
    .optional()
    .isNumeric()
    .withMessage('Rule till value must be a number')
    .isInt({ min: 1 })
    .withMessage('Rule till value must be a positive integer'),
...
const validateVote = [
  body('vote')
    .isIn(['like', 'dislike'])
    .withMessage('Vote must be either like or dislike'),
